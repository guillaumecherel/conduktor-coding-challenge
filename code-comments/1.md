...
// It should also throw a more specific exception than "Exception" if the
// function throws one that the application can recover from (see below).
...
public HttpStatus doSomething(JsonNode data) throws Exception {
    ...
    // have a "billing_user" field. For example, a class User and a
    // constructor "public Optional<User> create(JsonNode userInfo)",
    // that only returns some value if "userInfo" has all the data
    // required and None otherwise, and an accessor function "public
    // String getBillingAddress(User user)". The "unsafe code" handling
    // the outside world's messiness would then be localized in
    ...
    if (billingUserEmail.contains("@conduktor.io")) {
        ...
        // Does emailService.call always succeed? It might be a good idea to
        // handle its return value or exceptions thrown that could reflect 
        // network issues or other problems. For example, if it returns an
        // HttpStatus, return that rather than HttpStatus.OK.

...
public User createOrUpdateUser(String name, String email, int role, int money) throws Exception {
    ...
    // does one thing. To perform an action that either create or update a user,
    // the createOrUpdateUser function can be constructed by composing these
    // three functions. The advantage is that the "createUser" or "updateUser"
    // functions can be reused in other parts of the code.
    
    // The comments about type safety and immutability from 2.md also apply
    // here.
